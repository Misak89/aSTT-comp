...
- last_updated_utc: 2026-04-02T21:59:40Z
...
## Cíl aplikace
Primarni cil je merit a ladit STT modely pro **online mikrofonni prepis** cestiny (a pozdeji dalsich jazyku).

1. Mereni a tuning ruznych STT modelu pri zdroji z online mikrofonu.
2. Simulace mikrofonniho prepisu je povolena pro reprodukovatelne testy, ale musi byt vzdy explicitne oznacena jako simulace (nikdy ne jako online stream).
3. Na zalozce `/transcript` je cil prepis delsich zdroju (cca 30-240 minut).
4. U `/transcript` je akceptovane zpozdeni cca 30-60 s; vyrazne delsi zpozdeni je nezadouci.
5. Diagnosticky panel Dashboard ma slouzit k pochopeni HW naroku ruznych STT nastaveni a modelu.

**Klicovy pozadavek pro online rezim:**
- Audio jde do STT v real-time chuncich (typicky 100-200 ms) jako z mikrofonu.
- Model musi dlouhodobe stihat tempo vstupu (RTF < 1.0 = stiha, RTF > 1.0 = nestiha).

Vstup: online mikrofon, simulovane mikrofonni scenare s ground truth, dlouhe zdroje pro `/transcript`.
Vystup: WER/CER, latence, RTF, HW naroky a porovnani model x nastaveni x scenar.
...
## Dokumentacni Kontrakt (MUST)
- Pred jakymkoli zasahem do dokumentace nejdriv vytvor centralni timestamp backup do `docs/backups/<YYYY-MM-DD_HHMMSS>/`.
- V backupu zachovej relativni strukturu puvodnich souboru (napr. `root/AGENTS.md`, `docs/session_log.md`).
- Pro novy ne-core dokumentacni artefakt je povinny triplet `MD + JSON + JSONL`.
- `MD` je pouze lidska cteci vrstva (narativ), ne canonical strojovy format.
- `JSON` je canonical strukturovany snapshot.
- `JSONL` je append-only timeline/provenance stream.
- `TXT` (napr. rychla napoveda jako `scripts/log_cmd_help.txt`) neni soucast tripletu, pokud nejde o docs artefakt pod `docs/`.
- U novych docs s metadaty musi `doc_file` odpovidat nazvu souboru:
  - `MD`: `Doc-Meta -> doc_file`
  - `JSON`: `doc_meta.doc_file`
  - `JSONL`: `doc_meta.doc_file` v prvnim zaznamu.
- U snapshot/report docs se pouziva suffix data v nazvu (`_YYYY-MM-DD`), pokud nejde o stabilni living doc.
- Core/living governance dokumenty a systemove state soubory maji explicitni vyjimky podle `docs/DOCS_GOVERNANCE.md`.
...
│   ├── log_cmd.py             # Windows process logger (JSONL + MD)
│   ├── log_cmd_unix.py        # Linux/Mac process logger
│   └── log_cmd_help.txt       # rychla multilingual napoveda
├── docs/
...
# benchmark jobs se spousti pres API/UI; worker bezi jako subprocess (`scripts/benchmark_worker.py`)
```
