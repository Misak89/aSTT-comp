...
|---|---|---:|---:|---|---|---|
| P0 | permissions | 88 | 88 | stable | `warning: unable to access 'C:\Users\adamf/.config/git/ignore': Permission denied` | Check sandbox/escalation rule and filesystem access before rerun. |
| P0 | timeout | 19 | 19 | stable | `command timed out after 10155 milliseconds` | Tune timeout + split long operations into smaller validated steps. |
...
| P0 | permissions | 5 | 5 | stable | `failed to create root command: failed to read configuration: open C:\Users\adamf\AppData\R` | Check sandbox/escalation rule and filesystem access before rerun. |
...
| P0 | permissions | 1 | 1 | stable | `+        "keywords": ["permission denied", "access is denied", "spawn eperm"],` | Check sandbox/escalation rule and filesystem access before rerun. |
| P0 | permissions | 1 | 1 | stable | `ERROR: Could not install packages due to an OSError: [Errno 13] Permission denied: 'C:\\Us` | Check sandbox/escalation rule and filesystem access before rerun. |
| P1 | unknown | 12 | 12 | stable | `AttributeError: 'Wave_write' object has no attribute '_file'` | Classify manually and add/prefer repeatable guard condition. |
