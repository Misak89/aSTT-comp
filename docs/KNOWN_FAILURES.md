...
- owner: engineering
- status: active
- last_updated_utc: 2026-04-01T17:52:43Z
...
- total_events: 998
- unique_templates: 35
- promoted_rules_this_run: 0

## Top Recurring Failures

| Priority | Category | Count | Recent7d | Trend | Template | Action |
|---|---|---:|---:|---|---|---|
| P0 | permissions | 64 | 64 | stable | `warning: unable to access 'C:\Users\adamf/.config/git/ignore': Permission denied` | Check sandbox/escalation rule and filesystem access before rerun. |
...
| P0 | service_unavailable | 6 | 4 | stable | `API Error: Unable to connect to API (ConnectionRefused)` | Verify service health/port first, then retry workflow. |
...
| P0 | permissions | 3 | 3 | stable | `failed to create root command: failed to read configuration: open C:\Users\adamf\AppData\R` | Check sandbox/escalation rule and filesystem access before rerun. |
...
| P1 | timeout | 5 | 5 | stable | `command timed out after 10155 milliseconds` | Tune timeout + split long operations into smaller validated steps. |
| P1 | path_assumption | 2 | 0 | stable | `FileNotFoundError: [Errno 2] No such file or directory: '/c/Users/adamf/OneDrive/Dokumenty` | Validate path type/existence before operation (file vs directory). |
| P1 | path_assumption | 2 | 2 | stable | `NotADirectoryError: [WinError 267] The directory name is invalid: 'C:\\Users\\adamf\\OneDr` | Validate path type/existence before operation (file vs directory). |
| P1 | path_assumption | 1 | 0 | stable | `ls: cannot access 'C:\Users\adamf\OneDrive\Dokumenty\aSTT-comparison\apps\backend\app" 2>&` | Validate path type/existence before operation (file vs directory). |
