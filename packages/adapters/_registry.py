    ...
    m.model_id: m for m in [
        ...
        ModelDescriptor(
            ...
            params=[
                ...
                ParamSpec("threads",        "Vlákna CPU",     "int",  8,    "Počet CPU vláken", min=1, max=16),
                ParamSpec("beam_size",      "Beam size",      "int",  1,    "Větší = přesnější, pomalejší", min=1, max=10),
                ParamSpec("best_of",        "Best of",        "int",  1,    "Počet kandidátů", min=1, max=10),
                ParamSpec("no_fallback",    "Bez fallbacku",  "bool", False, "Zakáže fallback na menší model"),
                ParamSpec("initial_prompt", "Kontext (prompt)", "str",  "Aspergerův syndrom",   "Tip: krátký věcný kontext zlepší přesnost — např. 'Psychiatr Jiří Horáček, rozhovor o mozku:'. Příliš dlouhý prompt může škodit."),
                ParamSpec("analysis_interval_ms", "Mic interval (ms)", "int", 2000, "Jak často se aktualizuje live přepis", min=300, max=5000),
                ...
                ParamSpec("backpressure_high_s", "Backpressure high (s)", "float", 2.0, "Prahový dluh zpracování; nad touto hodnotou začne drop chunků", min=0.2, max=5.0),
                ParamSpec("backpressure_low_s", "Backpressure low (s)", "float", 0.8, "Hystereze; pod touto hodnotou se backpressure vypne", min=0.05, max=3.0),
            ],
        ),
        ...
        ModelDescriptor(
            ...
            params=[
                ...
                ParamSpec("chunk_seconds",  "Chunk délka",      "float", 0.40,  "Délka audio chunku v sekundách", min=0.05, max=2.0),
                ParamSpec("set_words",      "Word timestamps",  "bool",  False, "Vrátit timestamps pro každé slovo"),
                ParamSpec("input_gain_db", "Mic gain (dB)", "float", 1.0, "Softwarové zesílení/zeslabení vstupu před STT", min=-24.0, max=24.0),
                ParamSpec("backpressure_high_s", "Backpressure high (s)", "float", 1.4, "Prahový dluh zpracování; nad touto hodnotou začne drop chunků", min=0.2, max=5.0),
                ParamSpec("backpressure_low_s", "Backpressure low (s)", "float", 0.5, "Hystereze; pod touto hodnotou se backpressure vypne", min=0.05, max=3.0),
            ],
        ),
